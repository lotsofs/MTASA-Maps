...
addEventHandler ( "onPlayerJoin", _root, function ()
	...
	for id, player in ipairs(getElementsByType("player")) do
		if ( hasObjectPermissionTo ( player, "general.adminpanel" ) ) then
			triggerClientEvent ( player, "aClientPlayerJoin", source, "<hidden>" , getPlayerUserName ( source ), getPlayerAccountName ( source ), getPlayerSerial ( source ), hasObjectPermissionTo ( source, "general.adminpanel" ), aPlayers[source]["country"] )
		end
	end
...
addCommandHandler ( "register", function ( player, command, arg1, arg2 )
	...
	if ( password ~= nil ) then
		...
		elseif ( addAccount ( username, password ) ) then
			...
			outputServerLog ( "ADMIN: "..getPlayerName ( player ).." registered account '"..username.."' (IP: ".."<hidden>".."  Serial: "..getPlayerSerial(player)..")" )
...
addCommandHandler ( "unregister", function ( player, command, arg1, arg2 )
	...
	outputServerLog ( "ADMIN: "..getAdminNameForLog ( player ).." unregistering account '"..username.."' "..result.." (IP: ".."<hidden>".."  Serial: "..getPlayerSerial(player)..")" )
end )
	...
	elseif ( action == "aclcreate" ) then
		...
		if ( ( name ) and ( string.len ( name ) >= 1 ) ) then
			if ( arg[1] == "group" ) then
				...
				outputServerLog ("ACL: "..getPlayerName(source).."["..getAccountName (getPlayerAccount(source)).."] ["..getPlayerSerial (source).."] [".."<hidden>".."] created "..mdata)
				end
	...
	elseif ( action == "acldestroy" ) then
		...
		if ( arg[1] == "group" ) then
			if ( aclGetGroup ( name ) ) then
				...
				outputServerLog ("ACL: "..getPlayerName(source).."["..getAccountName (getPlayerAccount(source)).."] ["..getPlayerSerial (source).."] [".."<hidden>".."] destroyed "..mdata)
			else
				action = nil
			end
		elseif ( arg[1] == "acl" ) then
		...
		if ( arg[3] ) then
			...
			if ( arg[1] == "object" ) then
				...
				if ( not aclGroupAddObject ( group, object ) ) then
					...
					mdata2 = "Object '"..arg[3].."'"
				outputServerLog ("ACL: "..getPlayerName(source).."["..getAccountName (getPlayerAccount(source)).."] ["..getPlayerSerial (source).."] [".."<hidden>".."] added "..object.." to ACL Group "..arg[2])
			...
			elseif ( arg[1] == "acl" ) then
				...
				if ( not aclGroupAddACL ( group, acl ) ) then
					...
					mdata2 = "ACL '"..arg[3].."'"
					triggerEvent ( "aAdmin", source, "sync", "aclobjects", arg[2] )
				outputServerLog ("ACL: "..getPlayerName(source).."["..getAccountName (getPlayerAccount(source)).."] ["..getPlayerSerial (source).."] [".."<hidden>".."] added "..mdata2.." to Group "..arg[2])
				end
			elseif ( arg[1] == "right" ) then
				...
				outputServerLog ("ACL: "..getPlayerName(source).."["..getAccountName (getPlayerAccount(source)).."] ["..getPlayerSerial (source).."] [".."<hidden>".."] added "..mdata2.." to ACL "..arg[2])
				end
			end
		else
			action = nil
		...
		if ( arg[3] ) then
			...
			if ( arg[1] == "object" ) then
				...
				if ( not aclGroupRemoveObject ( group, object ) ) then
					...
					mdata2 = "Object '"..arg[3].."'"
					triggerEvent ( "aAdmin", source, "sync", "aclobjects", arg[2] )
				outputServerLog ("ACL: "..getPlayerName(source).."["..getAccountName (getPlayerAccount(source)).."] ["..getPlayerSerial (source).."] [".."<hidden>".."] removed "..mdata2)
				end
			elseif ( arg[1] == "acl" ) then
				local group = aclGetGroup ( arg[2] )
				local acl = aclGet ( arg[3] )
			...
			elseif ( arg[1] == "right" ) then
				...
				if ( not aclRemoveRight ( acl, right ) ) then
					...
					mdata2 = "Right '"..arg[3].."'"
					triggerEvent ( "aAdmin", source, "sync", "aclrights", arg[2] )
				outputServerLog ("ACL: "..getPlayerName(source).."["..getAccountName (getPlayerAccount(source)).."] ["..getPlayerSerial (source).."] [".."<hidden>".."] removed "..mdata2.." from "..mdata)
				end
			end
		else
			action = nil
	...
	if ( hasObjectPermissionTo ( source, "command."..action ) ) then
		...
		elseif ( action == "ban" ) then
			...
			if bUseSerial then
				...
				outputChatBox ( "You banned IP " .. "<hidden>", source, 255, 100, 70 )
				if isAnonAdmin then
					setTimer ( function()
...
addEventHandler ("onUnban",root,function(theBan,responsibleElement)
	if isElement(responsibleElement) and getElementType (responsibleElement)=="player" then
		outputServerLog ("BAN: "..getPlayerName(responsibleElement).."["..getAccountName (getPlayerAccount(responsibleElement)).."] ["..getPlayerSerial (responsibleElement).."] [".."<hidden>".."] unbanned player "..(getBanNick(theBan) or "unknown").." ["..(getBanIP (theBan) or getBanSerial(theBan)).."] ")
	end
end)

addEventHandler ("onBan",root,function(theBan)
	if isElement(source) and getElementType (source)=="player" then
		outputServerLog ("BAN: "..getPlayerName(source).."["..getAccountName (getPlayerAccount(source)).."] ["..getPlayerSerial (source).."] [".."<hidden>".."] banned player "..(getBanNick(theBan) or "unknown").." ["..(getBanIP (theBan) or getBanSerial(theBan)).."] ")
	end
end)
