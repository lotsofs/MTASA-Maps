...
addEventHandler("doShowPoll", root,
	function (pollData, pollOptions, pollTime)
		...
		local width = math.max( pollBoxRectDefault.width, dxGetTextWidth(pollTitle.."  Time left: 99", (screenX/1080) - 0.25, "default-bold", true) + 20 )
	...
	if drawPoll then
		...
		for index, option in ipairs(pollOptionsText) do
			...
			if pollMaxVoters ~= 0 then
				dxDrawRectangle (pollBoxRect.x + 12, pollBoxRect.y + 37 + (40 * index), (pollBoxRect.width - 24) * ((pollVotes[index] / pollMaxVoters)*1.9), 6, tocolor ( (pollVotes[index] / pollMaxVoters)*355, 0, 0, 255), true)
			end
		end
	end
end
