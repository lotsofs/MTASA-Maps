...
function dumpMeta ( xml, extraNodes, resource, filename, test )
	...
	-- LotsOfS: This is jank. mapContainer is fetched from... somewhere????? It certainly isn't local to this function.
	local objectLODpairs = { [0] = 0 }
	for i, child in ipairs(getElementChildren(mapContainer)) do
		local m = getElementModel(child)
		if (m) then
			objectLODpairs[m] = getLODModelFromObjectModel(m) or nil
		end
	end
	
	local scriptName = "newMapEditorScriptingExtension_s.lua"
	local scriptContent = SCRIPTING_EXTENSION_CODE .. "\n" .. getLodsLuaTableString(objectLODpairs)
