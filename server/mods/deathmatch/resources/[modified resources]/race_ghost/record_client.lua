...
function GhostRecord:saveGhost( rank, time )
	-- LotsOfS: We're now storing everything. No longer need to check for toptime, however we wíll pass whether this is the case along
	-- Also removing hex codes from names
	local topTime = time < globalInfo.bestTime and rank == 1
	local personalBest = time < globalInfo.personalBest
	if (not topTime and not personalBest) then return end -- Remove this line to save all runs, not just PBs
	triggerServerEvent( "onGhostDataReceive", getLocalPlayer(), self.recording, time, removeColorCoding(getPlayerName(getLocalPlayer())), self.resourceName, topTime, personalBest)
end
