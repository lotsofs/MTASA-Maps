	...
	elseif map and not gamemode then
		...
		if exports.mapmanager:isMapCompatibleWithGamemode(map, runningGamemode) then
			...
			return (startPoll{
				...
				[2]={"No",outputVoteManager,"votemap: not enough votes to set next map to '"..mapName.."'.",root,vR,vG,vB;default=true},
			}), true
		else
			return false, errorCode.mapIsntCompatible
		end
...
function findMaps( query, gamemode )
	...
	for i,resource in ipairs(maps) do
		...
		-- Find match for query within names
		if string.find( infoName:lower(), query:lower(), 1, true ) then
			table.insert( results, resource )
		end
		if string.find( resName:lower(), query:lower(), 1, true ) then
			table.insert( results, resource )
		end
	end
	return results
