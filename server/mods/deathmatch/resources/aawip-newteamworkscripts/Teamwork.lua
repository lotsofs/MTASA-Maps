...
-- ----
-- ----
-- packer1 is placed weirdly
-- people wait for hte big drop
-- people go in the dumper cargo hold instead of aiwaitn
-- dumper gets stuck on exit with cargo door up
-- diagonal bus gets stuck -- ensure straighter entry
-- between dumper & beagle can get stuck in the hole
-- drop into the packer area flips you nastily
-- add repair on dft drop
-- add shortcut into the baseball area from the start, to the dft area. 
-- yankee on the way to where its to be parked can sometimes hit the right caddy sapwn, move them further away. hhitboxes in this game suck
-- go faster up the helper ramp
-- > redesign the start area, see if helper upramp can be on the right instead of left, and then provide a shortcut to DFT.
-- > Maybe change from caddy to quad? Should be fine, should only be minimal speed bump required.
-- > currently a billboard is blocking a quick way from the DFT exit to the helper catchup gate. PErhaps allow this shortcut.
...
PLAY_AREA = createColRectangle(1257,2067,214,233)			-- bounding rectangle of the play area. Players leaving this area will be flagged as wanderers. 


HELPER_AREA = createColCuboid(1443, 2076, 0, 14, 154, 30)	-- if riders enter this area, they will get a prompt to respawn (eg. they fell off the track or wandered out of a shared area)
...
CAMERA_POSITION_X = 1369.2
CAMERA_POSITION_Y = 2279.6
CAMERA_POSITION_Z = 16.5
CAMERA_TARGET_X = 1354.3
CAMERA_TARGET_Y = 2274.7
CAMERA_TARGET_Z = 13.5

POSSIBLE_TEAMS = {											-- possible team names & base colors. This list will be shuffled.
	{"Hot Dogs with Mayo",255,255,255,"#FFFFFF"}, --white
	{"The Hot Dog Burners",0,0,0,"#000000"}, -- black
	{"Hot Dogs with Ketchup",255,0,0,"#FF0000"}, -- red
	{"Field Team",0,255,0,"#00FF00"}, -- green
	{"True Sportsmanship",0,0,255,"#0000FF"}, -- blue
	{"Team Outdoor Sports",0,255,255,"#00FFFF"}, -- cyan
	{"Spoiled Hot Dogs",255,0,255,"#FF00FF"}, -- magenta
	{"Hot Dogs with Mustard",255,255,0,"#FFFF00"} -- yellow
}
