...
function aBanInputBox ( player )
	...
	if ( aBanInputForm == nil ) then
		...
		aBanInputRadio1C		= guiCreateRadioButton ( 50, 50, 190, 15, "Both", false, aBanInputRadioSet1 )
...
function aBanInputBoxFinish ()
	-- Get options
	local bUseIP	 = guiRadioButtonGetSelected( aBanInputRadio1A ) or guiRadioButtonGetSelected( aBanInputRadio1C )
	local bUseSerial = guiRadioButtonGetSelected( aBanInputRadio1B ) or guiRadioButtonGetSelected( aBanInputRadio1C )
	...
	triggerServerEvent ( "aPlayer", localPlayer, aBanInputPlayer, "ban", reason, seconds, bUseSerial, bUseIP )
