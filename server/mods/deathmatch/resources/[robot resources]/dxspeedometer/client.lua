...
function toggleDebugEnabled()
	...
	outputChatBox("[dxspeedometer] Debug "..(s("debug") and "enabled" or "disabled"))
end
addCommandHandler("dxspeedometer_toggleDebug", toggleDebugEnabled)

-----------------
-- Temp Hiding --
-----------------

local playerNotRacing = false
local screenFadedOut = false
local showingPoll = false

-- Need to add events, otherwise it doesn't necessarily work
addEvent("onClientPlayerOutOfTime", false)
addEvent("onClientPlayerFinish", false)
addEvent("onClientMapStarting", false)
addEventHandler("onClientPlayerOutOfTime",getRootElement(),function() playerNotRacing = true end)
addEventHandler("onClientPlayerFinish",getRootElement(),function() playerNotRacing = true end)
addEventHandler("onClientMapStarting",getRootElement(),function() playerNotRacing = false end)

-- Also added like this in race_client.lua, not sure if remote trigger has to be allowed or not
...
draw = function()
	if (s("debug")) then
		dxDrawText("[Idle] "..tostring(playerNotRacing).." [Fade] "..tostring(screenFadedOut).." [Poll] "..tostring(showingPoll),0,0)
	end
	if not s("enabled") or playerNotRacing or screenFadedOut or showingPoll then
		return
	end
...
function createGui()
	...
	guiMemoSetReadOnly(gui.helpMemo, true)
	guiHelpMemo = gui.helpMemo

	gui.buttonSave = guiCreateButton( 0, 530, 260, 20, "Save Settings To File", false, gui.window )
	addHelp("Saves current settings to a file, so that they stay the same when you reconnect.",gui.buttonSave)
